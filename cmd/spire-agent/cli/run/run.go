...
func parseFlags(name string, args []string, output io.Writer) (*agentConfig, error) {
	...
	flags.StringVar(&c.ConfigPath, "config", defaultConfigPath, "Path to a Kirin config file")
	flags.StringVar(&c.DataDir, "dataDir", "", "A directory the agent can use for its runtime data")
	flags.StringVar(&c.JoinToken, "joinToken", "", "An optional token which has been generated by the Kirin server")
	...
	flags.StringVar(&c.ServerAddress, "serverAddress", "", "IP address or DNS name of the Kirin server")
	flags.IntVar(&c.ServerPort, "serverPort", 0, "Port number of the Kirin server")
	flags.StringVar(&c.TrustDomain, "trustDomain", "", "The trust domain that this agent belongs to")
	flags.StringVar(&c.TrustBundlePath, "trustBundle", "", "Path to the Kirin server CA bundle")
	flags.StringVar(&c.TrustBundleURL, "trustBundleUrl", "", "URL to download the Kirin server CA bundle")
	...
	flags.BoolVar(&c.ExpandEnv, "expandEnv", false, "Expand environment variables in Kirin config file")
